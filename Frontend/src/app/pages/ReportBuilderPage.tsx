...
export function ReportBuilderPage({
  ...
  const reportMetrics = useMemo(() => {
    ...
    const filteredCases = cases.filter((c) => {
      ...
      if (window) {
        ...
        if (
          t < window.from.getTime() ||
          t > window.to.getTime()
        )
          return false;
      }

      if (selectedLine !== "all" && c.line_id !== selectedLine)
        return false;
      if (
        selectedGroup !== "all" &&
        c.product_group !== selectedGroup
      )
        return false;
      return true;
    });
    ...
    const ngCount = filteredCases.filter(
      (c) => c.decision === "NG",
    ).length;
    const reviewCount = filteredCases.filter(
      (c) => c.decision === "REVIEW",
    ).length;
    const okCount = filteredCases.filter(
      (c) => c.decision === "OK",
    ).length;

    const ngRate =
      total > 0 ? ((ngCount / total) * 100).toFixed(2) : "0.00";
    const reviewRate =
      total > 0
        ? ((reviewCount / total) * 100).toFixed(2)
        : "0.00";
    const passRate =
      total > 0 ? ((okCount / total) * 100).toFixed(2) : "0.00";

    const avgScore =
      total > 0
        ? (
            filteredCases.reduce(
              (sum, c) => sum + c.anomaly_score,
              0,
            ) / total
          ).toFixed(3)
        : "0.000";
    ...
    filteredCases
      ...
      .forEach((c) => {
        defectBreakdown[c.defect_type] =
          (defectBreakdown[c.defect_type] || 0) + 1;
      });

    // Representative cases (worst cases)
  ...
  const defectChartData = useMemo(() => {
    ...
    return Object.entries(reportMetrics.defectBreakdown).map(
      ([type, count]) => ({
        ...
        <div className="grid grid-cols-5 gap-6 mb-6">
          ...
          <div className="bg-red-50 rounded-lg p-4">
            ...
            <p className="text-3xl font-semibold text-red-700">
              {reportMetrics.ngCount}
            </p>
            <p className="text-xs text-red-600 mt-1">
              {reportMetrics.ngRate}%
            </p>
          </div>
          <div className="bg-amber-50 rounded-lg p-4">
            ...
            <p className="text-3xl font-semibold text-amber-700">
              {reportMetrics.reviewCount}
            </p>
            <p className="text-xs text-amber-600 mt-1">
              {reportMetrics.reviewRate}%
            </p>
          </div>
          <div className="bg-green-50 rounded-lg p-4">
            ...
            <p className="text-3xl font-semibold text-green-700">
              {reportMetrics.okCount}
            </p>
            <p className="text-xs text-green-600 mt-1">
              {reportMetrics.passRate}%
            </p>
          </div>
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-3xl font-semibold text-blue-700">
              {reportMetrics.avgScore}
            </p>
            <p className="text-xs text-blue-600 mt-1">
              {reportMetrics.avgInferenceTime}ms
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ul className="space-y-2 text-sm text-blue-900">
            <li className="flex items-start gap-2">
              ...
              <span>
                전체 불량률은{" "}
                <strong>{reportMetrics.ngRate}%</strong>로
                {parseFloat(reportMetrics.ngRate) > 2.5
                  ? " 목표치(2.5%)를 초과했습니다."
                  : " 목표치(2.5%) 이하를 유지하고 있습니다."}
              </span>
            </li>
            {Object.entries(reportMetrics.defectBreakdown)
              .length > 0 && (
              <li className="flex items-start gap-2">
                <span className="text-blue-600 mt-0.5">•</span>
                <span>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={hourlyTrend}>
              ...
              <Bar dataKey="재검" fill="#f59e0b" stackId="a" />
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        ...
        {reportMetrics.representativeCases.length > 0 ? (
          <div className="space-y-4">
            {reportMetrics.representativeCases.map(
              (caseData, idx) => (
                <div
                  key={caseData.id}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {caseData.timestamp.toLocaleString(
                          "ko-KR",
                        )}{" "}
                        · {caseData.line_id}
                      </p>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={caseData.decision}
                        size="sm"
                      >
                        {caseData.decision}
                      </Badge>
                      <Badge
                        variant={caseData.severity}
                        size="sm"
                      >
                        {caseData.severity === "high"
                          ? "높음"
                          : caseData.severity === "med"
                            ? "중간"
                            : "낮음"}
                      </Badge>
                    </div>
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">
                        {caseData.defect_type === "seal_issue"
                          ? "실링 불량"
                          : caseData.defect_type ===
                              "contamination"
                            ? "오염"
                            : caseData.defect_type === "crack"
                              ? "파손/균열"
                              : caseData.defect_type === "dent"
                                ? "찌그러짐"
                                : "스크래치"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium text-red-600">
                        {caseData.anomaly_score.toFixed(3)}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500 text-xs">
                      ...
                      <p className="font-medium text-gray-900">
                        {caseData.affected_area_pct.toFixed(1)}%
                      </p>
                    </div>
                  </div>
